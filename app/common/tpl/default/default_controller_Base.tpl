...
// 初始化后台模块常量
$this->init{$modules}Const();

// 初始化后台模块信息
$this->init{$modules}Info();
...
* 初始化后台模块信息
*/
final private function init{$modules}Info()
{
...
// 检查菜单权限
list($jump_type, $message) = $this->logic{$module}Base->authCheck(URL_MODULE, $this->authMenuUrlList);
...
// 获取过滤后的菜单树
$this->authMenuTree = $this->logic{$modules}Base->getMenuTree($this->authMenuList, $this->authMenuUrlList);
...
* 初始化后台模块常量
*/
final private function init{$modules}Const()
{
...
return $this->logic{$modules}Base->filter($content, $this->authMenuUrlList);
}
}
