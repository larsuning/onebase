...
* PsInspection模型
*/
class PsInspection extends ApiBase
{
	public $type = array(
				'1' => '农残检测表',
				'2' => '标品收货记录表',
	...
	* 生成表数据
	...
	{  
		...
		$data = \think\Db::view('ps_inspection_record','goods_batch,record_type,record_date')
                            ->view('ps_inspection','record_id,goods_id,id,name,inspect_time,product_total,product_select,production_date,expire_date,unqualified_rate,is_qualified,is_certificate,remark,product_unit,unqualified_num','ps_inspection_record.id=ps_inspection.record_id')
							->view('provider',['name'=>'provider'],'ps_inspection.provider_id = provider.id')
							->where($where)
							->select();
                            // dd($data);
