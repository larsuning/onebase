 ...
 * 后台基类控制器
 */
class PsBase extends ControllerBase
{
    ...
    public function __construct()
    {
        ...
        // 后台控制器钩子
        Hook::listen('hook_controller_ps_base', $this->request);
    }
    
    /**
     * 初始化后台模块信息
     */
    final private function initpsInfo()
    {
        ...
        // 检查菜单权限
        list($jump_type, $message) = $this->logicPsBase->authCheck(URL_MODULE, $this->authMenuUrlList);
    ...
    final private function initBaseInfo()
    {
        ...
        // 获取过滤后的菜单树
        $this->authMenuTree = $this->logicPsBase->getMenuTree($this->authMenuList, $this->authMenuUrlList);
     ...
     * 初始化后台模块常量
     */
    final private function initpsConst()
    {
    ...
    final protected function fetch($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        return $this->logicPsBase->filter($content, $this->authMenuUrlList);
    }
