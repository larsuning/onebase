  ...
  <div class="content-wrapper">
    ...
    <div class="box-header">
      ...
      <?php if(empty($review) || (($review instanceof \think\Collection || $review instanceof \think\Paginator ) && $review->isEmpty())): ?>
        ...
        <?php endif; if(empty($review) || (($review instanceof \think\Collection || $review instanceof \think\Paginator ) && $review->isEmpty())): ?>
            <select name=""   class="form-control" style="float:right;width: 150px;font-weight: bold" id="storage_shift">
        ...
        <div class="box-body">
            <div class="row">
              <table class="table table-bordered" style="margin: 5px" id="storage">
                <thead>
                    <tr>
                      <!-- <th>#</th> -->
                      <th>区域</th>
                  ...
                  <?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?>
                      <tbody>
                          <tr><td colspan="7" style="text-align: center;height: 100px;line-height: 100px">当天的存储区巡查记录已经全部提交完毕!</td></tr>
                      </tbody>
                  <?php endif; if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?>
                    ...
                    <tr data-id="<?php echo $vo['id']; ?>">
                        ...
                        <td> <input type="text" value="<?php echo $vo['temperature']; ?>" name="temperature" /> </td>
                        <td> <input type="text" value="<?php echo $vo['humidity']; ?>" name='humidity'/> </td>
                        <td> <input type="text" value="<?php echo $vo['saving_status']; ?>" name="saving_status" /> </td>
                        <td> <input type="text" value="<?php echo $vo['clean_status']; ?>" name="clean_status" /> </td>
                    </tr>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                    </tbody>
                <?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
    ...
    if("<?php echo input('edit'); ?>"){
        $('#storage_shift').attr('disabled','on');
    }
    ...
    $('.finish').click(function(){
        ...
        _id = $('#storage tr').eq(1).attr('data-id');
        $.post('storage?action=recordEdit',{storage_id:_id},function(d){
            obalert(d);
        })
    })
    ...
    $('table').on('click','input[name="inspectioned_at"]',function(){
        ...
        $.post('storage',{inspectioned_at:_time,id:_id},function(d){
            console.log(d);
        })
    })
    ...
    $('table').on('change','input',function(){
        ...
        $.post('storage',data,function(e){
            console.log(e);
            return false;   
        })
    })

    $('#storage_shift').on('change',function(e){
        _val=$(this).val();
       _val=$(this).val();
